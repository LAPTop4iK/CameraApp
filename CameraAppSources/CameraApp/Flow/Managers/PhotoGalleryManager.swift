...
extension PhotoGalleryManager {
    func fetchPhotos(withIdentifiers identifiers: [String]) async throws -> [GalleryImageModel] {
        assetsFetchResult = PHAsset.fetchAssets(withLocalIdentifiers: identifiers, options: fetchOptions)

        var photos: [GalleryImageModel] = []
        for i in 0 ..< assetsFetchResult.count {
            let asset = assetsFetchResult.object(at: i)
            let assetItem = try await loadImage(for: asset)
            if let image = assetItem.0, let identifier = assetItem.1 {
                let aspectRatio = CGFloat(image.size.width) / CGFloat(image.size.height)
                photos.append(GalleryImageModel(image: image, isSelected: false, aspectRatio: aspectRatio, identifier: identifier))
            }
        }
        return photos
    }
